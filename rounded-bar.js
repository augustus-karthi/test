...
ChartJS.elements.Rectangle.prototype.draw = function() {
    ...
    for (let i = 1; i < 4; i++) {
      ...
      if(height < 0){
        ...
        ctx.moveTo(x_bl+radius, y_bl);
        ctx.lineTo(x_br-radius, y_br);
        console.log([x_br, y_br, x_br, y_br-radius]);
        ctx.quadraticCurveTo(x_br, y_br, x_br, y_br-radius);
        ctx.lineTo(x_tr, y_tr+radius);
        console.log([x_tr, y_tr, x_tr-radius, y_tr]);
        ctx.quadraticCurveTo(x_tr, y_tr, x_tr-radius, y_tr);
        ctx.lineTo(x_tl+radius, y_tl);
        console.log([x_tl, y_tl, x_tl, y_tl+radius]);
        ctx.quadraticCurveTo(x_tl, y_tl, x_tl, y_tl+radius);
        ctx.lineTo(x_bl, y_bl-radius);
        console.log([x_bl, y_bl, x_bl+radius, y_bl]);
        ctx.quadraticCurveTo(x_bl, y_bl, x_bl+radius, y_bl);
